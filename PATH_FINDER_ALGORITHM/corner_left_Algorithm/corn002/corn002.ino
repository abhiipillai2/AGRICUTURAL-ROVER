...
//calulate_all_distance
  ...
  if(YD_PING_DISTACE > total_margin){
    ...
    bot_engine_stop();
    //bot puller
    bot_stebilizer(GLOBEL_X,GLOBEL_Y,ORNT_2);
    delay(500);
    //stop engin
    
    Y_PING_DISTACE = sonar_Y.ping_cm();
    delay(1000);
    //forward up to margin
...
//*************************************************************************************************************************************
//bot puller
void BOT_PULLER (int x,int y,int ornt){
  
int internal_y = 0;
int internal_x = 0;
//for calculating all distance based on which is the orientation
  if(ornt == 1){
  ...
  //calulate_all_distance
  delay(1000);
  X_PING_DISTACE = sonar_YD.ping_cm();
  delay(1000);
  Y_PING_DISTACE = sonar_X.ping_cm();
  delay(1000);
  XD_PING_DISTACE = sonar_Y.ping_cm();
  delay(1000);
  YD_PING_DISTACE = sonar_XD.ping_cm();
  delay(1000);
...
diff_value_y = (internal_y - y);
abs_value_y = abs(diff_value_y);

diff_value_x = (internal_x - x);
abs_value_x = abs(diff_value_x);
//take puller 

if((abs_value_y - GLOBEL_Y) > 2 || (abs_value_x - GLOBEL_X) > 3 ){

        //call stebilizer function
        bot_stebilizer(x,y,ornt);
