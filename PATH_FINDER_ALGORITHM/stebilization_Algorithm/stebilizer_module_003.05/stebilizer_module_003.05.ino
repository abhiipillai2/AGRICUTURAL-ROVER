...
void bot_stebilizer (int X , int Y, int ornt){
  ...
  delay(1000);
      ...
      while(!(internal_y <= GLOBEL_Y && internal_x <= GLOBEL_X)){
        ...
        if(internal_y == GLOBEL_Y && internal_x == GLOBEL_X){
 ...
 //calulate_all_distance
  ...
  internal_y = (Y_PING_DISTACE + YD_PING_DISTACE + Y_OFFSET);
  internal_x =(X_PING_DISTACE + XD_PING_DISTACE + X_OFFSET);
  Serial.println(internal_x);
  Serial.println(internal_y);
  ref_y = internal_x;

  delay(1000);

      //stebilizer loop
      while(!(internal_y <= GLOBEL_X && internal_x <= GLOBEL_Y)){
        
        //stop
        bot_engine_stop();

        //delay weigth calculation
        Temp_w = (internal_x - GLOBEL_Y);
        int fix_offset = 0;
        fix_offset = abs(Temp_w);
        W = (fix_offset * 50);
        DW = (W0 + W );
        ...
        if(counter_flag){
          ...
          if(internal_x > ref_y || internal_x > ref_y + 5 || internal_x > ref_y + 10 || internal_x > ref_y + 15 || internal_x > ref_y + 20 ) {
              rot_flag = false;
              counter_flag = false; 
              ref_y = internal_x;
          } 
        ...
        //check brek conditions
        if(internal_x == GLOBEL_Y && internal_y == GLOBEL_X ){
