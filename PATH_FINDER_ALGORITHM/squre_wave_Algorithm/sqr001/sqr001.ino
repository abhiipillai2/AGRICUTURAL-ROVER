...
if(Y_PING_DISTACE > YD_PING_DISTACE){
  ...
  int new_xd = 0;
   ...
   new_x = X_PING_DISTACE + X_OFFSET;
   new_xd = XD_PING_DISTACE - X_OFFSET;
   
  if(XD_PING_DISTACE > total_margin){
    
    while(!(X_PING_DISTACE > new_x)){
...
//delay weigth calculation
        ...
        //second check
        delay(1000);
        X_PING_DISTACE = sonar_XD.ping_cm();
        if(X_PING_DISTACE == new_x){
            break;
        }    
    }
