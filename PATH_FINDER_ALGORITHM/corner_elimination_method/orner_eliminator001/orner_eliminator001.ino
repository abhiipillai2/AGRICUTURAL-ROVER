...
//calulate_all_distance
  ...
  while(!(calculator_distance >= ref_distance)){
    ...
    //distance calculatio
    calculator_distance = (Y_PING_DISTACE + YD_PING_DISTACE + Y_OFFSET);
    corner_eliminator = true;
  }

  //loop-2 for eliminating corner
  while(corner_eliminator){
    //stop
    bot_engine_stop();
    //left rotate
    bot_anti_clock(500);
    ...
    calculator_distance = (Y_PING_DISTACE + YD_PING_DISTACE + Y_OFFSET);
        ...
        if(calculator_distance > (ref_distance - 4) && calculator_distance < (ref_distance) + 3){
